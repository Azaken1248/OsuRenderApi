...
async def _upload_log_and_fail(
    s3, bucket_name: str, job_id: str, log_path: str, error: str
) -> dict:
    ...
    try:
        async with session.client(
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                                ...
                                try:
                                    ...
                                    if dl.status_code == 200 and len(dl.content) > 100:
                                        with open(osz_path, "wb") as f:
                                            f.write(dl.content)
                                        log(
                                            f"  Downloaded {len(dl.content)} bytes from {url}"
                                        )
                                        downloaded = True
                                        ...
                                        try:
                                            ...
                                            await s3.upload_file(
                                                osz_path, bucket_name, beatmap_s3_key
                                            )
                                        ...
                                        log(
                                            f"  Failed: status {dl.status_code}, size {len(dl.content)}"
                                        )
                                except Exception as e:
                                    log(f"  Error: {e}")
                                    continue
                        if not downloaded:
                            return await _upload_log_and_fail(
                                s3,
                                bucket_name,
                                job_id,
                                shared_log_path,
                    ...
                    if not os.path.exists(os.path.join(skin_folder, "skin.ini")):
                        ...
                        try:
                            await s3.download_file(
                                bucket_name, f"skins/{skin}.osk", skin_tmp_path
                            )
                ...
                log(
                    f"Danser output ({len(danser_output)} chars):\n{danser_output[-2000:]}"
                )

                pp_gained = 0.0
                try:
                    match = re.search(
                        r"\|\s*1\s*\|(?:[^|]*\|){11}\s*([\d.]+)\s*\|", danser_output
                    )
                    if match:
