...
- Consider creating a `.env` file and using `python-dotenv` to load sensitive variables during development.
- Add unit/integration tests for API endpoints that create render jobs.
- Add a health-check endpoint and PM2 ecosystem restart policies for production.

## License

See repository license (if present). If none, add a `LICENSE` file.

---

If you want, I can also:

- Add an example `.env` file and load it from `app.py`.
- Update `ecosystem.config.js` with a concrete interpreter path for a created venv.
- Create a small verification script to check `DANSER_PATH` and `OSU_API_KEY` on startup.
