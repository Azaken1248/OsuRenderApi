...
- **Message Broker (Redis)**: Queues rendering jobs to distribute workloads.
- **Task Workers (Celery)**: Consume jobs from Redis, download necessary assets, and orchestrate rendering.
...
- **Object Storage (S3 / MinIO)**: Stores `.osr` replays, `.mp4` videos, thumbnails, and rendering logs.
...
- A [Modal](https://modal.com) account (for GPU rendering capabilities)
...
cp .env.example .env
```
Ensure you provide a valid `OSU_API_KEY` (available at https://osu.ppy.sh/p/api) and configure Modal properly.

### 2. Deploy Local Infrastructure and Application
...
```
*Note: Wait a few seconds for MinIO to initialize and the `minio-init` container to provision the required storage buckets.*

### 3. Deploy Modal GPU Worker

If `USE_MODAL_GPU=1` is set in your `.env` configuration, you must deploy the cloud GPU worker:
...
```
*Note: Ensure your MinIO instance is accessible from the internet to allow Modal to upload the generated artifacts.*

## Local Development

For developing locally without Docker:
...
Legacy endpoints (`/render`, `/jobs`, `/status`) are strictly supported for backward compatibility with the legacy monolithic application.
